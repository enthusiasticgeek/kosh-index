...
| ~~**vani-symbolic**~~ ✅ v0.2.0 published 2026-07-25 | vani-bignum | Expression trees, simplification rules, symbolic differentiation/integration, equation solving. v0.1.0 (construction/eval/print, 22 functions) and v0.2.0 (simplification -- constant folding, identities, flatten-and-collect like-term collection for flat linear combinations of monomials, 26 functions) both published; verified via a fresh `vanic add symbolic` in a scratch project across all four run/build × backend combinations (namespaced calls, e.g. `symbolic::sym_simplify`, `symbolic::ExprNode`). v0.2.0's validation leaned on property-based random sampling (`seed_rng`/`rand_in_range`), not just hand-picked examples, per the roadmap's own explicit call for this on the highest-risk phase. See the scoping breakdown below for the full phased plan; v0.3.0+ not started. |
