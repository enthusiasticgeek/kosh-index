...
| ~~**vani-symbolic**~~ ✅ v0.1.0 published 2026-07-25 | vani-bignum | Expression trees, simplification rules, symbolic differentiation/integration, equation solving. v0.1.0 (construction/eval/print, 22 functions) published; verified via a fresh `vanic add symbolic` in a scratch project across all four run/build × backend combinations (namespaced calls, e.g. `symbolic::sym_add`, `symbolic::ExprNode`). See the scoping breakdown below for the full phased plan; v0.2.0+ not started. |
