...
| [sparse](https://github.com/enthusiasticgeek/vani-sparse) | 0.1.3 | Sparse matrices: COO (build) / CSR (operate) formats, dense conversions, matvec (binary-search element lookup), transpose, scale, add, matmul (Gustavson's algorithm); every op cross-checked against the equivalent dense vani-matrix operation |
